...
async def test_session_build_resumed_restores_history(
    ...
    """Session.build_resumed loads a prior session's history + profile."""
...
async def test_session_build_resumed_raises_when_workspace_gone(
    ...
    store.save_meta(workspace=str(gone), profile="alpha", started_at="t")
    ...
    with pytest.raises(SessionResumeError):
        Session.build_resumed(session_id="gone1234", profile_arg=None,
                              ui_sinks=sinks)
