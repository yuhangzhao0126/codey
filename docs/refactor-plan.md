...
Carve-outs from §16 and §17 below are explicit changes; everything else is preserved.
...
- The 4 always-on built-in hooks (`permission`, `audit_log`, `transcript`, `stop_logger`) plus `todo_nag` + `todo_render` keep firing on the same events. §16 changes what `transcript` writes and extends what `audit_log` records; nothing else.
...
## 16. Behavior change — quiet TUI + persistent tool log + opt-in OTel tracing

This is an intentional, scoped behavior change layered on top of the structural refactor. Ships as **PR-C** after PR-B lands.

### 16.1 What changes in the TUI

Today the transcript shows three kinds of tool-related lines per call:

```
  → bash(command='git status')              ← call line (yellow)
  ← bash [ok]                                ← result header (green/red)
      On branch main…                        ← result body (dim)
```

After PR-C, **all three are gone from the TUI**. The transcript shows only:

- `you ›` user lines
- `codey›` assistant replies (rendered per §17)
- Todo-list updates (unchanged)
- `[turn finished: …]` meta lines (unchanged)
- Approval modals (unchanged — still pop up when the permission engine says ask)

Rationale: tool noise crowds the actual conversation. Power users who want to see what's happening can `tail -f` the log (16.2) or attach an OTel viewer (16.3).

**Implementation:**

- The `transcript` built-in hook (`hooks/builtin/transcript.py`) stops calling the UI writer for `PreToolUse` and `PostToolUse`. Its writer hookup is removed from `build_default_hooks(...)` when the host is the TUI.
- The `_log_tool_call` / `_log_tool_result` helpers in `ui/renderers.py` are deleted (they have no other callers after the hook is detached).
- Approval modals are unaffected — they're driven by the `permission` hook, not `transcript`.

Tests in `tests/test_tui.py` that assert tool lines appear in the RichLog get updated to assert their *absence*.

### 16.2 Extended audit log (always on)

`audit_log` already writes `~/.cache/codey/calls.jsonl` with one line per `PreToolUse` / `PostToolUse`. Two changes:

- **Stop truncating `result` at 500 chars.** Today the file stores `result_preview` (capped) + `result_truncated` flag + `result_chars`. After PR-C: store the full `result` field. (The truncation existed to keep the file small; for a personal-use agent the disk cost is negligible, and being able to grep the full output is exactly what makes "no tool lines in TUI" tolerable.)
- **Add a `session_id` field** on every line — a short uuid generated once at `Session.build()` time. Lets `jq 'select(.session_id == "abc123")' calls.jsonl` filter to one run.

New line shape:

```json
{
  "ts": "2026-06-04T14:32:11",
  "session_id": "8f3a2b",
  "event": "PostToolUse",
  "tool": "bash",
...
The old `result_preview` / `result_truncated` / `result_chars` fields go away. (One-line breaking change in the JSONL schema, called out here so any user-side `jq` scripts get updated.)

**Implementation:**

- `hooks/builtin/audit_log.py` — drop the truncation branch, drop the three preview fields, add `session_id` from a new `session_id` argument to the hook factory.
- `core/session.py:Session.build()` generates `self.session_id = uuid.uuid4().hex[:8]` and passes it into `build_default_hooks(...)`.
- `tests/test_audit_log.py` (NEW, small) — verify the full result is written and that `session_id` is present.

### 16.3 Opt-in OpenTelemetry tracing hook

A new built-in hook `hooks/builtin/otel.py` that emits OTel spans for every model round + tool call. Off by default; opt in via env var or flag:

- `CODEY_OTEL=1` (env var), OR
- `--otel` (CLI flag added to `app.py`), OR
- `[otel] enabled = true` block in `~/.config/codey/config.toml`

When enabled, the hook reads `OTEL_EXPORTER_OTLP_ENDPOINT` (default `http://localhost:4318`) and `OTEL_SERVICE_NAME` (default `codey`) — standard OTel env vars, so any OTel-aware backend (Jaeger, Tempo, Phoenix, Honeycomb, Datadog) Just Works.

**Span shape:**

```
turn (session_id, profile.name, model)
└─ round 0 (round_index)
   ├─ model_request (provider.base_url, model)        ← span from streaming.py
   ├─ tool_call: bash (tool, call_id, arguments)      ← span from the otel hook
   │  ├─ permission_check (decision: allow/deny/ask)
   │  └─ tool_exec (ok, result_chars)                 ← result attached as attribute
   └─ tool_call: read_file …
└─ round 1 …
```

`turn` and `round` spans are started/ended from `core/turn.py` (the agent loop). The `tool_call`-nested spans are started by the OTel hook in `PreToolUse` and ended in `PostToolUse`. The hook keeps a `dict[call_id, Span]` to bridge the two events.
...
Installed with `uv sync --extra observability`. The base install stays lean. `hooks/builtin/otel.py` imports OTel lazily inside the factory so the package loads fine without the extra; the factory raises a friendly error if `CODEY_OTEL=1` is set but the extra isn't installed.

**Suggested local viewer:** [Arize Phoenix](https://github.com/Arize-ai/phoenix) — purpose-built for LLM agent tracing, one-line docker run, shows prompts/responses/tool calls in a treeview. Document this in README's new "Debugging" section.

**Tests:** `tests/test_otel_hook.py` (NEW, small) — boots the hook with a mock exporter, runs one fake turn, asserts the span tree shape (one `turn`, one `round`, one `tool_call` per dispatch). Skipped if the `observability` extra isn't installed.

### 16.4 Migration sequence for PR-C

12. **Extend `audit_log`** — full results, add `session_id` (requires Session to exist, so step 12 depends on step 8 / PR-B). Update tests. **Gate:** green.

13. **Detach `transcript` hook from the TUI** (and delete the now-unused `_log_tool_call` / `_log_tool_result` helpers). Update `tests/test_tui.py` to assert tool lines are absent. **Gate:** green.

14. **Add the OTel hook** as an opt-in built-in. Add the `observability` extra to `pyproject.toml`. Add `--otel` flag to `app.py`. Add the new test (skipped unless extra installed). Update README + CLAUDE.md with a Debugging section explaining the env var, the flag, and a sample `docker run phoenix` line. **Gate:** green.

### 16.5 Risk register additions

| Risk | Likelihood | Mitigation |
|---|---|---|
| Users relying on `result_preview` field in old `calls.jsonl` scripts break | low (personal project) | Call out in README. Provide a one-liner migration jq if needed. |
| OTel hook + a misconfigured collector hangs `Stop` event waiting for export | medium | Use `BatchSpanProcessor` (async, drops if backed up) not `SimpleSpanProcessor` (sync). Set a 1s export timeout. |
| Lazy OTel import error message is confusing | low | Friendly error in the hook factory: "OTel tracing requested but the 'observability' extra isn't installed. Run: uv sync --extra observability" |

---

## 17. Behavior change — Markdown rendering for codey's replies

Layered into PR-C. Today `_log_assistant` writes the raw model text as one line of Rich markup. Replies that contain markdown (headers, lists, inline `code`, fenced code blocks, tables) render as plain text — structure is invisible.

### 17.1 What changes

Render codey's reply as Markdown using `rich.markdown.Markdown`. This handles:

- `# Headers` → sized headers
- `**bold**`, `*italic*`
- Bullet and numbered lists with proper indent
- `` `inline code` `` → background-tinted monospace
- ```` ```language\n…\n``` ```` fenced code blocks → syntax-highlighted via Pygments (already a Rich dep, no new install)
- Tables, blockquotes, horizontal rules

This matches Claude Code, codex, and aider. No new dependency — Rich + Pygments are already pulled in by Textual.

### 17.2 Streaming behavior (the one tradeoff)

You can't syntax-highlight a fenced code block whose closing ``` hasn't arrived yet, so we can't markdown-render mid-stream. The chosen design:

- **During the stream**, append text deltas as raw text under a `codey›` line (current behavior — keeps the "live typing" feel).
- **On `AssistantMessageCompleted`**, erase the raw block and write the full reply as `rich.markdown.Markdown(text)` in its place.

Net visual effect: the user sees the raw words appear as they're generated, then the block "settles" into rendered markdown at end-of-turn (~0.1s flash, like Claude Code's behavior). If the reply is a single short sentence with no markdown, the swap is imperceptible.

**Implementation (in `ui/streaming.py`):**

```python
async def stream_turn(app, user_input: str) -> None:
    raw_buf = ""
    raw_anchor = None     # line index of the in-progress raw reply, for replacement

    async for ev in app.session.agent.run(user_input):
        if isinstance(ev, AssistantTextDelta):
            if raw_anchor is None:
                raw_anchor = app.transcript.line_count
                app.transcript.write("[bold magenta]codey›[/]")
            raw_buf += ev.text
            app.transcript.replace_line(raw_anchor + 1, raw_buf)  # or append-then-replace

        elif isinstance(ev, AssistantMessageCompleted):
            # Swap: remove the raw block, write the markdown render in its place.
            app.transcript.remove_lines(raw_anchor, app.transcript.line_count)
            app.transcript.write("[bold magenta]codey›[/]")
            app.transcript.write(Markdown(ev.text))
            raw_buf = ""
            raw_anchor = None
...
(`replace_line` / `remove_lines` are pseudo-API; actual RichLog has `clear()` + write — we may need to maintain a small reply-window state and re-render. Detail to nail down in implementation, not now.)

### 17.3 Risks

| Risk | Likelihood | Mitigation |
|---|---|---|
| Markdown swap creates a visible flicker on long replies | medium | Acceptable per chosen design; matches Claude Code behavior. If it's bad in practice we can disable swap for replies under N chars (no flicker needed). |
| Models that emit broken markdown (unclosed code fences, malformed tables) render badly | low | `rich.markdown.Markdown` tolerates broken input; worst case renders as text. |
| Pygments lexer for an exotic language throws | low | Wrap the markdown render in try/except; on failure fall back to writing raw text. |

### 17.4 Migration sequence

Folded into PR-C step 13 (when we touch `ui/streaming.py` anyway to drop tool lines). One additional test: `tests/test_tui.py::test_assistant_reply_renders_markdown_at_end_of_turn`.

---

## 18. Revised PR plan

| PR | Commits | Scope | Gate |
|---|---|---|---|
| **PR-A** | 1 | §8 CLI deletion | tests pass (~124) |
| **PR-B** | 10 | §9 structural refactor (steps 2-11) | tests pass after each commit |
| **PR-C** | 3 | §16 quiet TUI + extended audit log + opt-in OTel; §17 markdown rendering | tests pass; manual smoke test on TUI for the markdown swap |

Total: 14 commits across 3 PRs. Each PR is independently mergeable and reversible.

---

## 19. Revised effort

- PR-A: 1-2 hours (delete + doc updates + test run).
- PR-B: ~1.5 dev days (10 commits, gating between each).
- PR-C: ~half day (3 commits + a small amount of OTel exploration if you've never wired it up before).

Total: ~2 dev days. Agent-executed with `uv run pytest` between each step: ~1 day.
